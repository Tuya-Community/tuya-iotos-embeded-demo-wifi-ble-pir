 ...
 * @LastEditTime: 2021-03-25 11:33:50
 */
...
OPERATE_RET app_pir_init(IN APP_MODE mode)
{
...
VOID app_report_all_dp_status(VOID)
{
    ...
    dp_arr[1].dpid = DPID_SET_SENS;
    dp_arr[1].type = PROP_ENUM;
    dp_arr[1].time_stamp = 0;
    dp_arr[1].value.dp_value = app_ctrl_data.sens;

    dp_arr[2].dpid = DPID_SET_ONTIME;
    dp_arr[2].type = PROP_ENUM;
    dp_arr[2].time_stamp = 0;
    dp_arr[2].value.dp_value = app_ctrl_data.ontime;
...
VOID deal_dp_proc(IN CONST TY_OBJ_DP_S *root)
{
    ...
    case DPID_SET_SENS:
        app_ctrl_data.sens = root->value.dp_enum;
        break;
    case DPID_SET_ONTIME:
        app_ctrl_data.ontime = root->value.dp_enum;
        break;
    default:
        break;
    }
